...
Resolves extracted entity/topic strings to actual graph nodes.
...
class Resolver:
    ...
    async def _resolve_entities(
        self, hints: list[str], question: str, top_k: int
    ) -> list[str]:
        ...
        all_resolved = []
        for hint in hints:
            candidates = await self._get_entity_candidates(hint, top_k)
            if not candidates:
                log(f"No entity candidates for: {hint}")
                continue

            resolved = await self._verify_candidates(hint, candidates, "EntityNode", question)
    ...
    async def _resolve_topics(
        ...
        all_resolved = []
        for hint in hints:
            candidates = await self._get_topic_candidates(hint, top_k)
            if not candidates:
                log(f"No topic candidates for: {hint}")
                continue

            resolved = await self._verify_candidates(hint, candidates, "TopicNode", question)
            all_resolved.extend(resolved)

        return list(set(all_resolved))

    async def _get_entity_candidates(self, hint: str, top_k: int) -> list[str]:
        """Get candidate EntityNodes via vector similarity search."""
        # Embed the hint
        embedding = await asyncio.to_thread(
            self.services.embeddings.embed_query, hint
        )
    ...
    async def _get_topic_candidates(self, hint: str, top_k: int) -> list[str]:
