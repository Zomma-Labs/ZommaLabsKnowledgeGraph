"""
V6 GraphStore: Unified interface for all graph queries.
...
NOTE: This file is identical to V5 - no changes needed for threshold-only retrieval.
The embedding service (services.embeddings) automatically uses OpenAI text-embedding-3-large.
"""
...
TOPIC_RESOLUTION_SYSTEM_PROMPT = """You are resolving a topic term to knowledge graph topic nodes.

Given a topic term and candidate topic nodes, select ONLY the single most relevant topic.

RULES:
- Return exactly ONE topic - the best semantic match
- Be precise: if the term is "economic growth", match "Economic Activity" not tangentially related topics like "Employment" or "Consumer Spending"
- Only match topics that directly represent the concept, not topics that are merely related
- If no candidates are a good match, set no_match to true"""
...
class GraphStore:
    ...
    async def _resolve_topics(
        ...
        """Resolve topic hints to TopicNode names - returns only top match per hint."""
        if not hints:
            return []
        ...
        for hint in hints:
            ...
            # Use topic-specific resolution (top match only)
            resolved_name = await self._verify_topic_candidate(display_name, candidates, question)
            if resolved_name:
    ...
    async def _verify_topic_candidate(
        self,
        hint: str,
        candidates: list[str],
        question: str,
    ) -> Optional[str]:
        """LLM verification for topics - returns only the single best match."""
        if not candidates:
            return None
        ...
        try:
            result = await asyncio.to_thread(
                self.structured_resolver.invoke,
                [
                    ("system", TOPIC_RESOLUTION_SYSTEM_PROMPT),
                    ("human", prompt),
                ]
            )

            if result.no_match:
                log(f"LLM found no topic match for '{hint}'")
                return None

            # Return only the first/best match
            if result.resolved_nodes:
                best = result.resolved_nodes[0].name
                log(f"Resolved topic '{hint}' -> {best}")
                return best
            return None

        except Exception as e:
            log(f"Topic resolution error for '{hint}': {e}")
            # Fallback to top candidate only
