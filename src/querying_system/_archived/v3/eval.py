"""
Evaluate V3 Threshold-Based Pipeline
=====================================

Usage:
    uv run python -m src.querying_system.v3.eval --limit 10
    uv run python -m src.querying_system.v3.eval --threshold 0.6 --limit 10
"""
...
async def evaluate_question(pipeline: ThresholdPipelineV3, judge, question: str, expected: str, q_id: int, category: str) -> dict:
    ...
    try:
        ...
        if facts:
            min_score = min(f.vector_score for f in facts)
            max_score = max(f.vector_score for f in facts)
            avg_score = sum(f.vector_score for f in facts) / len(facts)
        else:
        ...
        num_facts = 0
        confidence = 0
        decomp_time = retrieval_time = synthesis_time = 0
        min_score = max_score = avg_score = 0
...
async def run_evaluation(qa_pairs: list, threshold: float, max_concurrent: int = 3):
    """Run evaluation with concurrency."""
    total = len(qa_pairs)

    print(f"\n{'='*70}", flush=True)
    print(f"V3 THRESHOLD-BASED EVALUATION", flush=True)
    print(f"Questions: {total} | Threshold: {threshold} | Concurrency: {max_concurrent}", flush=True)
    print(f"{'='*70}\n", flush=True)
    ...
    async def eval_with_semaphore(i, pair):
        async with semaphore:
            ...
            print(f"[{completed[0]}/{total}] Q{q_id}: {icon} ({result['agent_time_sec']:.1f}s, {result['num_facts']} facts, avg_sim={result['avg_score']:.3f}) - {category}", flush=True)
...
def print_summary(results: list, threshold: float):
    ...
    avg_sim = sum(r["avg_score"] for r in results) / total

    print(f"\n{'='*70}")
    print("V3 THRESHOLD-BASED RESULTS")
    print(f"{'='*70}")
    print(f"\n  Threshold: {threshold}")
...
def save_results(results: list, threshold: float, output_path: str):
    ...
    data = {
        ...
        "summary": {
            ...
            "avg_facts": sum(r["num_facts"] for r in results) / total,
            "avg_similarity": sum(r["avg_score"] for r in results) / total,
            "avg_confidence": sum(r["confidence"] for r in results) / total,
        },
        "results": results
    }
...
async def main():
    ...
    results = await run_evaluation(qa_pairs, threshold=args.threshold, max_concurrent=args.concurrency)
    print_summary(results, args.threshold)

    output = args.output or f"eval_v3_t{args.threshold}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    save_results(results, args.threshold, output)
