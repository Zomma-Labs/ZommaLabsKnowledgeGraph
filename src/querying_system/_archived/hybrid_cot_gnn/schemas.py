...
class QueryDecomposition(BaseModel):
    ...
    # Step 3: Hints for direct graph lookup
    entity_hints: list[str] = Field(
        default_factory=list, description="Entity names to resolve directly in graph"
    )
    topic_hints: list[str] = Field(
        default_factory=list, description="Topic/theme names to search for"
    )
