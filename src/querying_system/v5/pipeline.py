"""
V5 Pipeline: Entity-Anchored Deep Research Orchestrator.
    ...
    async def _research_parallel(
        ...
        async def bounded_research(sq) -> SubAnswer:
            async with semaphore:
                ...
                return await researcher.research(sq, question_context, question_type)
...
async def query_v5(
    ...
    """
    Convenience function to run a single query through V5 pipeline.

    Example:
        result = await query_v5("What economic conditions did Boston report?")
        print(result.answer)
    """
    pipeline = V5Pipeline(group_id=group_id, config=config)
