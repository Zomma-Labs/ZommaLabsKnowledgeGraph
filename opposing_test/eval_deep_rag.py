"""
Evaluate Deep Research + RAG on Full QA List
=============================================

Tests the multi-agent architecture with simple RAG backend.

Usage:
    uv run python opposing_test/eval_deep_rag.py
"""
...
from opposing_test.deep_research_rag import DeepResearchRAGPipeline
...
async def run_evaluation(qa_pairs: list, max_concurrent: int = 5):
    """Run evaluation on all questions with concurrency."""
    total = len(qa_pairs)

    print(f"\n{'='*70}")
    print(f"DEEP RESEARCH + RAG EVALUATION")
    print(f"Questions: {total} (concurrency: {max_concurrent})")
    print(f"{'='*70}\n")

    pipeline = DeepResearchRAGPipeline()
    judge = get_judge()
...
def print_summary(results: list):
    ...
    print(f"\n{'='*70}")
    print("DEEP RESEARCH + RAG RESULTS")
    print(f"{'='*70}")
...
async def main():
    ...
    output = args.output or f"eval_deep_rag_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
