...
class DeferredDeduplicationManager:
    ...
    def _build_connected_components(self,
                                     ...
                                     similarity_threshold: float = 0.70) -> Dict[int, List[int]]:
        """
    ...
    def _llm_dedupe_batch(self, entities: List[PendingEntity]) -> List[DistinctEntity]:
        ...
        prompt = f"""You are an expert in financial entity deduplication for a knowledge graph.
...
- Legal name ↔ trade name: "Apple Inc." = "AAPL"
- Ticker symbol ↔ company name: "MSFT" = "Microsoft"
- Formal name ↔ nickname: "Federal Reserve" = "The Fed"

NEVER MERGE different entities, even if related:
- Person != Company they lead: "Tim Cook" ≠ "Apple Inc." (he's the CEO, not the company!)
- Person != Company in their title: "Apple CEO Tim Cook" is a PERSON, not "Apple Inc."
- Competitors in same industry: "Goldman Sachs" ≠ "Morgan Stanley"
- Parent != subsidiary: "Alphabet" ≠ "YouTube"
...
- Think about what TYPE of thing each entity is (person vs company vs index etc.)
...
- THINK twice before merging. We would rather have more groups than wrong merges.
...
Return the distinct real-world entities found."""
    ...
    def cluster_and_remap(self, similarity_threshold: float = 0.70) -> Dict[str, Any]:
        """
            ...
            for comp_id, member_indices in components.items():
                ...
                for de in distinct:
                    if len(de.member_indices) > 1:
                        # Pick the first member as canonical (or could pick by longest summary)
