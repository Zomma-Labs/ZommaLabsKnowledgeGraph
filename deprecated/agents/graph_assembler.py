...
FACT_DEDUP_TOP_K = int(os.getenv("FACT_DEDUP_TOP_K", "3"))  # Number of candidates to check for deduplication
FACT_DEDUP_THRESHOLD = float(os.getenv("FACT_DEDUP_THRESHOLD", "0.90"))  # Similarity threshold for merging
...
class GraphAssembler:
    ...
    def assemble_fact_node(self, 
        ...
        candidates = self.neo4j.vector_search("fact_embeddings", fact_embedding, top_k=FACT_DEDUP_TOP_K, filters={"group_id": group_id})
        ...
        for i, pc in enumerate(precomputed):
            ...
            if fact_embedding:
                ...
                candidates = self.neo4j.vector_search(
                    "fact_embeddings", fact_embedding, top_k=FACT_DEDUP_TOP_K, filters={"group_id": group_id}
                )

                if candidates:
                    best = candidates[0]
                    if best['score'] > FACT_DEDUP_THRESHOLD:
                        # LLM verification
