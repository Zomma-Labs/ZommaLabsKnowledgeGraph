...
- **Example**: `explore_neighbors(entity_name="[Company Name]")` →
  ```
  outgoing:
    ACQUIRED[N]: [Subsidiary1],[Subsidiary2]...
    [EDGE_TYPE][N]: [Target1],[Target2]...
  incoming:
    FOUNDED_BY[N]: [Person1],[Person2]
  ```
- **IMPORTANT**: Use the edge types from this output for Step 3. If you see `[EDGE_TYPE][N]`, you can use that edge type in `get_chunks_by_edge`.
...
2. `explore_neighbors("[Exact Entity Name]")` → See available edges:
   - outgoing: [EDGE_TYPE1][N], [EDGE_TYPE2][N], ...
